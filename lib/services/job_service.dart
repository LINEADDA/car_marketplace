...
class JobService {
  ...
  Future<JobPosting?> getJobPostingById(String id) async {
    final response = await _client.from('job_postings').select().eq('id', id).maybeSingle();
  ...
  Future<void> toggleJobPostingStatus(String postId, bool currentStatus) async {
    await _client.from('job_postings').update({'is_active': !currentStatus}).eq('id', postId);
  }
  ...
  Future<SkilledWorker?> getSkilledWorkerById(String id) async {
    final response = await _client.from('skilled_workers').select().eq('id', id).maybeSingle();
