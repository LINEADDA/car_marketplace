...
class _SparePartDetailPageState extends State<SparePartDetailPage> {
  ...
  Future<void> _fetchPartDetails() async {
    ...
    try {
      ...
      if (mounted) setState(() { _part = part; _isLoading = false; });
    } catch (e) {
      if (mounted) setState(() { _errorMessage = 'Failed to load part details: ${e.toString()}'; _isLoading = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        actions: [
          if (_isOwner) ...[
            IconButton(icon: const Icon(Icons.edit_outlined), tooltip: 'Edit Part', onPressed: _navigateToEditPage),
            IconButton(icon: const Icon(Icons.delete_forever_outlined, color: Colors.redAccent), tooltip: 'Delete Part', onPressed: _handleDeletePart),
          ]
        ],
      ),
      body: _buildBody(),
          ...
          if (part.mediaUrls.isNotEmpty)
            SizedBox(
              ...
              child: PageView.builder(
                ...
                itemBuilder: (context, index) => Image.network(part.mediaUrls[index], fit: BoxFit.cover, errorBuilder: (c, o, s) => const Icon(Icons.broken_image, size: 100)),
              ),
            )
          else
            Container(height: 250, color: Colors.grey[300], child: const Center(child: Icon(Icons.build_circle_outlined, size: 100, color: Colors.grey))),
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(part.title, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                Text(currencyFormatter.format(part.price), style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                const Divider(),
                _buildSectionTitle('Condition'),
                Chip(label: Text(part.condition.name), backgroundColor: Colors.blueGrey[50]),
                const SizedBox(height: 16),
                _buildSectionTitle('Description'),
                Text(part.description.isNotEmpty ? part.description : 'No description provided.', style: Theme.of(context).textTheme.bodyLarge),
              ],
            ),
          ),
        ],
